...
// Custom Preparation Toolbar
const PreparationToolbarContainer = styled.div`
  width: 100%;
  max-width: 36rem;
...
interface PreparationToolbarProps {
  disabled?: boolean;
  onSend: () => void;
...
function PreparationToolbar({ disabled, onSend, isProcessing, onPromptSelect }: PreparationToolbarProps) {
    ...
    <PreparationToolbarContainer>
      ...
      <PromptsDropdown $isOpen={showPrompts}>
        ...
        <PromptsMenu $isOpen={showPrompts}>
          <PromptsMenuItem onClick={() => handlePromptSelect("Please Develop an Overview document")}>
            Please Develop an Overview document
          </PromptsMenuItem>
          <PromptsMenuItem onClick={() => handlePromptSelect("Please Develop a PRD")}>
            Please Develop a PRD
          </PromptsMenuItem>
          <PromptsMenuItem onClick={() => handlePromptSelect("Develop a task list; include task requests for each task (for Dev Team processing)")}>
            Develop a task list; include task requests for each task (for Dev Team processing)
          </PromptsMenuItem>
        </PromptsMenu>
      </PromptsDropdown>
...
const PreparationPage: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setPreparationResult(response);
    } catch (error) {
      setPreparationResult(`Error: ${error instanceof Error ? error.message : 'Failed to process request'}`);
    } finally {
      setPreparationProcessing(false);
    }
  };
  ...
  const handleExport = () => {
    if (preparationResult) {
      const blob = new Blob([preparationResult], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'preparation_result.txt';
      a.click();
      window.URL.revokeObjectURL(url);
    }
  };
  const handleClear = () => clearPreparationResult();
      ...
      <div className="relative w-full min-h-screen">
        <main className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 z-10">
          <div 
            ...
            onMouseEnter={() => setShowPreparationTooltip(true)}
            onMouseLeave={() => setShowPreparationTooltip(false)}
          ...
          >
            ...
            <div 
              ...
              className={`dev-team-tooltip ${showPreparationTooltip ? 'visible' : ''}`}
            >
              Project planning and task preparation
            </div>
          </div>

          <div className="flex items-center gap-8 relative">
            <div style={{ position: 'relative', zIndex: 2, marginTop: '100px' }}>
              <DogonMask isProcessing={preparationProcessing} />
            </div>
          </div>
          ...
          <div className="mt-[20px] w-full flex flex-col items-center gap-6 relative">
            <InputContainer>
              <TextInputArea
                value={prompt}
                onChange={setPrompt}
                disabled={preparationProcessing}
                onSend={handleSend}
              />
              <FileList />
              <PreparationToolbar
                ...
                onSend={handleSend}
                isProcessing={preparationProcessing}
                onPromptSelect={(prompt) => setPrompt(prompt)}
              />
            </InputContainer>
            {preparationResult && (
              <div className="w-full max-w-4xl mx-auto mt-6">
                <div className="bg-black rounded-lg p-6 border border-gray-700">
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: preparationResult.replace(/^Preparation Result:\s*/g, '') }}
                  />
